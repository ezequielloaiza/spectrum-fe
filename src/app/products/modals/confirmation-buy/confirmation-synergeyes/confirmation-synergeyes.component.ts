...
@Component({
  selector: 'app-confirmation-synergeyes',
  templateUrl: './confirmation-synergeyes.component.html',
  styleUrls: ['./confirmation-synergeyes.component.scss']
})
    ...
    if (this.typeBuy === 1) {
      ...
      this.basketService.saveBasket$(this.basketRequest).subscribe(res => {
        if (res.code === CodeHttp.ok) {
          ...
          console.log(res);
          this.translate.get('Connection Failed', { value: 'Connection Failed' }).subscribe((res1: string) => {
      ...
      if (this.client.status === StatusUser.InDefault) {
        ...
        // if (this.available) {
            ...
            if (res.code === CodeHttp.ok) {
              ...
              this.spinner.hide();
              this.close();
              this.translate.get('Order generated successfully', {value: 'Order generated successfully'}).subscribe(( res1: string) => {
                this.notification.success('', res1);
              });
